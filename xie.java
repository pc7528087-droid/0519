...
public class xie extends JFrame {
    ...
    public xie() {
        setTitle("Cartoon Filter Adjustable Version - Anime Style");
        setSize(1100, 700);
        ...
        // ★ 已經幫您將預設值調整為較接近卡通風格的數值
        edgeSlider = createSlider(50, 300, 90);    // 降低閾值，讓黑線更容易浮現
        colorSlider = createSlider(20, 120, 40);   // 色塊分層
        brightSlider = createSlider(0, 80, 30);    // 提亮
        blurSlider = createSlider(0, 5, 4);        // 增加模糊次數，抹平真實細節
    ...
    // ★ 加入 private 消除 Overridable method 警告
    ...
    // ★ 加入 private 消除 Overridable method 警告
    private JPanel makeControl(String name, JSlider slider) {
    ...
    void openImage() {
        ...
        if (r == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                processImage();
            } catch (IOException e) { // ★ 改為 IOException 捕捉特定錯誤
    ...
    void saveImage() {
        ...
        } catch (IOException e) { // ★ 同樣改為 IOException
    ...
    public static void main(String[] args) {
        // 確保 GUI 在事件調度執行緒中運行（好習慣）
        SwingUtilities.invokeLater(() -> {
            new xie().setVisible(true);
        });
    }
}
